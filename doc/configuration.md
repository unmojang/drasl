...
- `PlayerCertsLifetime`: Lifetime of an issued Minecraft player chat certificate, in seconds. The default is Mojang's 80-hour lifetime (`288000`). Set to `0` for a practically non-expiring certificate. Integer. Default value: `288000`.
- `PlayerCertsRefresh`: How long Drasl reuses a player's complete certificate response in memory before issuing a new key pair, in seconds. Public certificates remain in the database so later reports can verify historical messages; private keys are never stored. Set to `0` to issue a new certificate on every request. For a finite lifetime, this value must be shorter than `PlayerCertsLifetime`. Integer. Default value: `259200` (72 hours).
