...
- `Domain`: the fully qualified domain name where your instance is hosted. Clients using authlib-injector may not see skins if this option is not correctly set. String. Example value: `"drasl.example.com"`.
- `BaseURL`: the URL of your instance. String. Example value: `"https://drasl.example.com"`.
...
- `[CreateNewPlayer]`: Policy for existing Drasl users creating new players.
...
- `[[ImportExistingPlayer]]`: Allow existing Drasl users to import a player from a fallback API server. The UUID of the existing player will be used for the Drasl player.
...
- `[RegistrationUsernamePassword]`: Policy for password-based registration (i.e. users registering with a username and password, not via OIDC).
  - `[RegistrationUsernamePassword.CreateNewPlayer]`: Policy for registering a new Drasl account by creating a player with a new UUID.
    ...
    - `AllowChoosingUUID`: Allow users to choose a UUID for the new player. If disabled, the new player's UUID will always be generated according to the strategy specified by the `PlayerUUIDGeneration` option. Boolean. Default value: `false`.
  - `[[RegistrationUsernamePassword.ImportExistingPlayer]]`: Allow users to register a new Drasl account by importing an existing player from a fallback API server. Each entry references a fallback server by nickname.
    - `FallbackAPIServerNickname`: The `Nickname` of the `[[FallbackAPIServers]]` entry to import from. String.
...
- `MaxPlayerNameLength`: Users will not be able to choose usernames or player names longer than this length. Minecraft clients and servers may kick players whose names are longer than the vanilla limit of 16 characters. Raising this above 16 is only recommended if you expect your users will run a Yggdrasil agent on both the client and the server. Integer. Default value: `16`.
- `MinPlayerNameLength`: Users will not be able to choose usernames or player names shorter than this length. Must be at least `1` and no greater than `MaxPlayerNameLength`. Integer. Default value: `1`.
...
- `SignPublicKeys`: Whether to sign players' public keys. Boolean. Default value: `true`.
  - Not recommended when using fallback API servers.
  - Limits Minecraft servers' ability to forge messages from players.
  - Necessary to support Vanilla Minecraft servers with `enforce-secure-profile=true`.
  - Note: Vanilla Minecraft servers 1.19 and earlier can only validate player public keys against Mojang's public key, not ours, so you should use `enforce-secure-profile=false` on versions earlier than 1.20.
- `PlayerCertsLifetime`: Lifetime of an issued Minecraft player chat certificate, in seconds. Set to `0` to use Drasl's distant-future expiry (January 1, 2038). Chat report submission and storage of newly issued public certificates are disabled when this is `0` or shorter than `PlayerCertsRefresh`; name and skin reports remain available, as do existing reports. Equal lifetime and refresh values are allowed. Integer. Default value: `288000` (80 hours).
- `PlayerCertsRefresh`: How long Drasl reuses a player's complete certificate response in memory before issuing a new key pair, in seconds. Set to `0` to issue a new certificate on every request. Expired certificates are never served from the cache, even if their refresh deadline is later. Cached responses, including private keys, are removed on the next maintenance pass after refresh or expiry, normally within 10 minutes. Private keys are never stored in the database. Integer. Default value: `259200` (72 hours).
- `PlayerCertsRetention`: How long to retain public certificates after expiry, in seconds, to verify delayed chat reports. Set to `0` for cleanup without an additional grace period. Certificates referenced by retained reports, whether open or archived, are preserved until those reports are deleted. Cleanup runs every 10 minutes and does not immediately purge existing certificates when chat reporting is disabled. Certificate durations must be nonnegative and fit within Go's duration range. Integer. Default value: `43200` (12 hours).
...
- `AllowChangingPlayerName`: Allow users to change their "player name" after their account has already been created. Could be useful in conjunction with the `ImportExistingPlayer` configuration options if you want to make users register from an existing (e.g. Mojang) account but you want them to be able to choose a new player name. Admins can change the name of any player regardless of this setting. Beware: when `CreateNewPlayer.Allow = true` or there are `ImportExistingPlayer` entries, users can simply delete a player and create a new one with a new name, regardless of this setting. Boolean. Default value: `true`.
...
- `ValidPlayerNameRegex`: Regular expression (regex) that player names must match. Currently, Drasl usernames are validated using this regex too. Player names are also limited by `MinPlayerNameLength` and `MaxPlayerNameLength` no matter what this regex allows. Mojang allows the characters `abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_`, and by default, Drasl follows suit. Minecraft servers may misbehave if additional characters are allowed. Change to `.+` if you want to allow any player name of a permitted length. String. Default value: `^[a-zA-Z0-9_]+$`.
...
- `ClassicPublicIP`: IPv4 address to assign to [Classic server protocol heartbeat requests](https://minecraft.wiki/w/Classic_server_protocol#Heartbeats) when the originating IP is loopback or within a private IP range. Useful when Drasl runs on the same system as the Minecraft server it communicates with, allowing the heartbeat to report an externally reachable address instead of a local one. If not set, the origin IP will be used and the Classic server may not be joinable. String. Example value: `"1.1.1.1"`.
- `BlockedServers`: List of blocked Minecraft Server addresses. Clients that respect the [blocked servers API](https://minecraft.wiki/w/Mojang_API#Query_blocked_server_list) use this list to prevent players from connecting to them. Array of strings. Example value: `["bad.example.com", ".evil.net", "123.45.67.89"]`.
