...
// vanillaSkin describes a skin from Minecraft's DefaultPlayerSkin table.
...
// javaUUIDHashCode matches java.util.UUID.hashCode().
...
// VanillaDefaultSkin returns the skin that the Minecraft client assigns locally.
...
// httpReaderAt implements io.ReaderAt with HTTP range requests.
type httpReaderAt struct {
	url    string
	client *http.Client
}

// cachingReaderAt caches block-aligned reads from an io.ReaderAt.
...
// writeFileAtomic prevents an interrupted write from leaving a valid-looking file.
