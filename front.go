...
func (app *App) authenticationReturnURL(c *echo.Context) (string, error) {
...
func (app *App) BrowserAuthentication() func(echo.HandlerFunc) echo.HandlerFunc {
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(c *echo.Context) error {
			returnURL, err := app.authenticationReturnURL(c)
			if err != nil {
				return err
			}
...
func (app *App) BrowserRequireAuthentication() func(echo.HandlerFunc) echo.HandlerFunc {
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(c *echo.Context) error {
			if user, ok := c.Get(CONTEXT_KEY_MAYBE_USER).(mo.Option[User]).Get(); ok {
...
func (app *App) BrowserRequireAdmin() func(echo.HandlerFunc) echo.HandlerFunc {
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(c *echo.Context) error {
	...
	return func(c *echo.Context) error {
		...
		for _, name := range app.OIDCProviderNames {
			...
			webOIDCProviders = append(webOIDCProviders, webOIDCProvider{
				Name:          name,
				RequireInvite: oidcProviderRequiresInvite(&provider.Config),
			})
		}
...
func FrontCompleteRegistration(app *App) func(c *echo.Context) error {
	...
	return func(c *echo.Context) error {
		user := c.Get(CONTEXT_KEY_MAYBE_USER).(mo.Option[User])
...
// Mint a fresh OIDC state, set the state and (if applicable) PKCE cookies, and
// redirect to the provider's authorization endpoint.
func (app *App) beginOIDC(c *echo.Context, provider *OIDCProvider, action string, returnURL string) error {
	if returnURL == "" {
	...
	if parsed, err := url.Parse(returnURL); err == nil {
		state.Destination = parsed.Query().Get("destination")
		state.InviteCode = parsed.Query().Get("invite")
	}

	stateBase64, err := EncodeOIDCState(state)
	if err != nil {
		return err
	}
	...
	if provider.RelyingParty.IsPKCE() {
		...
		if err := provider.RelyingParty.CookieHandler().SetCookie(c.Response(), pkceCookieName(provider), codeVerifier); err != nil {
			return err
		}
	...
	authURL := rp.AuthURL(stateBase64, provider.RelyingParty, opts...)
	return c.Redirect(http.StatusSeeOther, authURL)
}

// GET /web/oidc-sign-in/:providerName
func FrontOIDCBeginSignIn(app *App) func(c *echo.Context) error {
...
// GET /web/oidc-link/:providerName
func FrontOIDCBeginLink(app *App) func(c *echo.Context) error {
	failureURL := app.FrontEndURL
	return func(c *echo.Context) error {
		providerName := c.Param("providerName")
		provider, ok := app.OIDCProvidersByName[providerName]
...
// A texture URL contains a hash of the texture, so it names the bytes exactly
// and works as an ETag.
...
func (app *App) getTextureBlob(textureURL string) ([]byte, error) {
	if filePath, ok := app.localTexturePath(textureURL); ok {
...
func (app *App) localTexturePath(textureURL string) (string, bool) {
	var directory, relative string
	if rest, ok := strings.CutPrefix(textureURL, app.TexturesURL+"/texture/"); ok {
		directory, relative = app.Config.StateDirectory, rest
	} else if rest, ok := strings.CutPrefix(textureURL, app.FrontEndURL+"/web/vanilla-skin/"); ok {
		directory, relative = app.GetVanillaDefaultSkinDirectory(), rest
	} else {
		return "", false
	}
	...
	// Texture URLs name a bare hash. The file on disk has the extension.
	if !strings.HasSuffix(unescaped, ".png") {
		unescaped += ".png"
	}

	// A fallback API server picks its own texture URLs. One that only imitates
	// ours must not reach a file outside the directory.
	relativePath := filepath.FromSlash(unescaped)
	if !filepath.IsLocal(relativePath) {
		return "", false
	}
	return filepath.Join(directory, relativePath), true
}
...
func frontChallenge(app *App, action string) func(c *echo.Context) error {
	...
	return func(c *echo.Context) error {
		...
		return c.Render(http.StatusOK, "challenge", challengeContext{
			...
			FallbackAPIServer: fallbackAPIServerConfig,
...
func FrontCreatePlayer(app *App) func(c *echo.Context) error {
	return func(c *echo.Context) error {
		...
		var fallbackAPIServerNickname *string
		if v := nilIfEmpty(c.FormValue("fallbackApiServer")); v != nil {
			fallbackAPIServerNickname = v
		}
