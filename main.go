...
func (app *App) MakeServer() *echo.Echo {
	...
	if app.Config.EnableWebFrontEnd {
		...
		texture.GET("/web/texture/missing-skin", FrontMissingSkin())
		texture.GET("/web/texture/resolved/:token", FrontResolvedTexture(app))
		texture.GET("/web/texture/player/:uuid/skin", FrontPlayerSkin(app))
		texture.GET("/web/texture/player/:uuid/cape", FrontPlayerCape(app))
		texture.GET("/web/texture/player/:uuid/preview.css", FrontPlayerPreviewCSS(app))
		texture.GET("/web/texture/user/:uuid/skin", FrontUserSkin(app))
		texture.GET("/web/vanilla-skin/:model/:name", FrontVanillaSkin(app))
		...
		web.GET("/web/oidc-sign-in/:providerName", FrontOIDCBeginSignIn(app))
