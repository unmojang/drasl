...
func (ts *TestSuite) testPlayerFallbackTextures(t *testing.T) {
	...
	auxPlayer.SkinModel = SkinModelSlim
	assert.Nil(t, ts.AuxApp.DB.Save(&auxPlayer).Error)

	_, browserTokenCookie := ts.CreateTestUser(t, ts.App, ts.Server, "fallbackUser")
	var user User
	...
	t.Run("the fallback skin and cape are previewed", func(t *testing.T) {
		...
		assert.Nil(t, err)
		assert.NotEmpty(t, preview.SkinURL)
		assert.True(t, preview.CapeURL.IsPresent())

		for _, textureURL := range []string{preview.SkinURL, preview.CapeURL.MustGet()} {
		...
		rec := ts.Get(
			t,
			ts.Server,
			"/web/texture/player/"+player.UUID+"/preview.css",
			[]http.Cookie{*browserTokenCookie},
			nil,
		)
		assert.Equal(t, http.StatusOK, rec.Code)
		assert.Contains(t, rec.Body.String(), "--skin-arm-width: 3")

		resolvedPath := regexp.MustCompile(`/web/texture/resolved/[A-Za-z0-9_-]+`).FindString(rec.Body.String())
		assert.NotEmpty(t, resolvedPath)
		rec = ts.Get(t, ts.Server, resolvedPath, []http.Cookie{*browserTokenCookie}, nil)
		assert.Equal(t, http.StatusOK, rec.Code)
	...
	t.Run("the player's own skin wins over the fallback", func(t *testing.T) {
		...
		assert.True(t, preview.CapeURL.IsAbsent())
	})
}
...
func (ts *TestSuite) testPreviewPlayerNoSkin(t *testing.T) {
	...
	preview, err := ts.App.GetPlayerPreview(player)
	assert.Nil(t, err)
	missingURL, err := ts.App.MissingSkinURL()
	assert.Nil(t, err)
	assert.Equal(t, missingURL, preview.SkinURL)
	assert.True(t, preview.CapeURL.IsAbsent())
...
func (ts *TestSuite) testPreviewFollowsArmModel(t *testing.T) {
	...
	assert.Equal(t, classicPreview.SkinURL, slimPreview.SkinURL)
...
func (ts *TestSuite) testPlayerAvatarSkinURLFollowsPreview(t *testing.T) {
	...
	missingURL, err := ts.App.MissingSkinURL()
	assert.Nil(t, err)
	assert.Equal(t, missingURL, avatar)
	...
	assert.Nil(t, err)
	assert.Nil(t, ts.App.SetSkinAndSave(&secondPlayer, bytes.NewReader(BLUE_SKIN)))
	secondAvatar, err := ts.App.PlayerAvatarSkinURL(&secondPlayer)
	assert.Nil(t, err)
	...
	assert.Equal(t, secondAvatar, *primary)
